...
#define LOGOG_THREAD std::thread

#define LOGOG_THREAD_CREATE(handle, attr, pStartFn, arg) \
	(handle = std::thread ( pStartFn, arg ))
...
#elif defined(LOGOG_FLAVOR_WINDOWS)
...
namespace RKUtils{
	namespace logog
	{
		...
		public:
			...
			{
#if defined(OS_W8) || defined(OS_WP8)|| defined OS_W10
				LOGOG_THREAD_CREATE(m_Thread, NULL, m_pFnThreadStart, m_pvThreadParams);
				return 0;
#else
			 ...
			 */
#if defined(OS_W8) || defined(OS_WP8)|| defined OS_W10
			static int WaitFor(Thread& thread)
			{
				LOGOG_THREAD_JOIN(thread.m_Thread);
				return 0;
			}
#else
